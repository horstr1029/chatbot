...
export function DarkModeToggle() {
  ...
  useEffect(() => {
    ...
    const isDark = stored ? stored === 'dark' : (isMobile ? false : prefersDark)
    setDark(isDark)
  ...
  return (
    <button
      ...
      className="p-1 rounded text-text-muted hover:text-text-secondary hover:bg-surface-secondary transition-colors"
    >
      {dark ? (
